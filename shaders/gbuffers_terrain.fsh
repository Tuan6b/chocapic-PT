...
vec4 getAlbedoTexture(sampler2D tex, vec2 coord, float bias) {
    vec4 albedo = texture2D(tex, coord, bias);
    #if WHITE_WORLD == 1
...
void main() {
    ...
    vec4 data0 = getAlbedoTexture(texture, lmtexcoord.xy, Texture_MipMap_Bias);
