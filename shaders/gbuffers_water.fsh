...
void main() {
	if (gl_FragCoord.x * texelSize.x < RENDER_SCALE.x  && gl_FragCoord.y * texelSize.y < RENDER_SCALE.y )	{
		...
		// Replace texture call with our white world aware function
		vec4 data = getAlbedoTexture(texture, lmtexcoord.xy);
		data *= color;  // Apply color after texture fetch
		
		float avgBlockLum = luma(texture2DLod(texture, lmtexcoord.xy,128).rgb*color.rgb);
		data.rgb = clamp((data.rgb)*pow(avgBlockLum,-0.33)*0.85,0.0,1.0);
		vec3 albedo = toLinear(data.rgb);
		if (iswater > 0.4) {
			albedo = vec3(0.42,0.6,0.7);
			data = vec4(0.42,0.6,0.7,0.7);
			#if WHITE_WORLD == 1
				albedo = vec3(1.0);
				data.rgb = vec3(1.0);
			#endif
		...
		direct *= (iswater > 0.9 ? 0.2: 1.0)*diffuseSun*lmtexcoord.w;
			...
			vec3 diffuseLight = direct + vec3(0.5);
		#else
		...
		if (iswater > 0.0){
		float f0 = iswater > 0.1?  0.02 : 0.05*(1.0-data.a);
		...
		data.a = -data.a*fresnel+data.a+fresnel;
		data.rgb =clamp(color/data.a*alpha0*(1.0-fresnel)*0.1+reflected/data.a*0.1,0.0,65100.0);
		if (data.r > 65000.) data.rgba = vec4(0.);
		}
		else
		data.rgb = color*0.1;
